...
IMPLICIT NONE 
!CONSTRUCTION THE ERROR VECTOR
!ER_T=Y(T)-TH(P+Q+1)-AR-MA
!TH=(PHI_1, ... , PHI_P, THETA_1,...,THETA_Q,C)
INTEGER, INTENT(IN):: T, P, Q
REAL*8,INTENT(IN):: Y(1:T), TH(1:P+Q+1)	!CONSTANT=TH(P+Q+1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
DOUBLE PRECISION FUNCTION FUNC(T,P,Q,Y,TH)!,EP)!
IMPLICIT NONE 
!TH=(PHI_1, ... , PHI_P, THETA_1,...,THETA_Q,C)
INTEGER, INTENT(IN):: T, P, Q
!
!REAL*8,INTENT(INOUT):: EP(1:T)
REAL*8:: EP(T)	
!
INTEGER:: I
REAL*8:: SUMAT		
REAL*8, INTENT(IN):: TH(P+Q+1), Y(1:T)		
SUMAT=0D0
...
REAL*8:: EP(1:T), DC(P+1:T), DPH(P+1:T,1:P), DTH(P+1:T)
INTEGER:: I,J,K
G=0.D0
...
DC=-1D0
DC(P+2)=-1D0+P0(P+1)
G(P+Q+1)=EP(P+1)*DC(P+1)+EP(P+2)*DC(P+2)	!FOR THE FIRST AND SECOND ELEMENTS OF THE G(P+Q+1)
DO I=P+3,T	!2
	IF (I .LE. P+Q) THEN 
		DO J=1, I-P-1 !2.1
	...
	ELSE
		...
		END DO		!2.2
	END IF
	G(P+Q+1)=G(P+Q+1)+EP(I)*DC(I)		!THE CONTINUEING  OF THE GRADIENT
END DO			!2
...
END DO	 !1
!4.!!!G(I) FINAL!!!!!!!!
G=2D0*G	!G=-2*G, FOR MAX. WE MULTIP. *-1
